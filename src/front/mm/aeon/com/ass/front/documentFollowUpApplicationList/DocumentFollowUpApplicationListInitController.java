...
public class DocumentFollowUpApplicationListInitController extends AbstractDAController implements
    ...
    public DocumentFollowUpApplicationListFormBean process(DocumentFollowUpApplicationListFormBean formBean) {
        ...
        formBean.setDocFollowUpTypeSelectItemList(super.getDocFollowUpStatusSelectList());
        if (formBean.getDocFollowUpTypeSelectItemList().size() == 0) {
            messageBean = new MessageBean(MessageId.ME1006, "Document Follow Up Status");
            messageBean.setMessageType(MessageType.ERROR);
            formBean.getMessageContainer().addMessage(messageBean);
            applicationLogger.log(messageBean.getMessage(), LogLevel.INFO);
        }
