...
public class ApplicationListInitController extends AbstractDAController
    ...
    public ApplicationListFormBean process(ApplicationListFormBean formBean) {
        ...
        formBean.setJudgementStatusSelectItemList(super.getJudgementStatusSelectList());
        if (formBean.getJudgementStatusSelectItemList().size() == 0) {
            messageBean = new MessageBean(MessageId.ME1006, "Judgment Status");
            messageBean.setMessageType(MessageType.ERROR);
            formBean.getMessageContainer().addMessage(messageBean);
            applicationLogger.log(messageBean.getMessage(), LogLevel.INFO);
        }

        formBean.setJudgementStatusSearchSelectItemList(super.getJudgementStatusSearchSelectList());
        if (formBean.getJudgementStatusSearchSelectItemList().size() == 0) {
            messageBean = new MessageBean(MessageId.ME1006, "Judgment Status For Search");
            messageBean.setMessageType(MessageType.ERROR);
            formBean.getMessageContainer().addMessage(messageBean);
            applicationLogger.log(messageBean.getMessage(), LogLevel.INFO);
        }
        ...
        applicationLogger.log("Application Init process ended.", LogLevel.INFO);
        return formBean;
    }
