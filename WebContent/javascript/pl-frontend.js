...
$(function() {
	...
	$("#search").autocomplete(
			{
				select : function(event, ui) {
					...
					clickRoom(phoneNoVal, msgHeaderVal, nrcNoVal);
					$("#search").val('');
	...
	if (window.location.port === '8082') {
		connection = new WebSocket('ws://' + window.location.hostname + ':8085')
	} else {
		connection = new WebSocket('wss://' + window.location.hostname
				+ '/vcschat')
	}*/
	var connection = new WebSocket('wss://ass.aeoncredit.com.mm/vcsplchat');
	// var connection = new WebSocket('ws://10.1.9.69:8085');
		...
		if (json.type === 'listroom') {
			...
			if (json.data.op_send_flag == 0 && json.data.room === phoneNo) {
				...
				connection.send("ChangeReadFlagWithMsgId:"
						+ json.data.pl_message_id);
			}
			if (messageId !== json.data.pl_message_id) {
			...
			messageId = json.data.pl_message_id;
			...
			if (oldMessage.length == 0) {
				...
				for (var i = 0; i < oldMessage.length; i++) {
					...
					if (i == oldMessage.length - 1) {
						msgid = oldMessage[i]["pl_message_id"];
					}
				}
			}
	...
	function addMessage(author, message, datetime, op_send_flag, isOldMsg,
		...
		if (message_type == 1) {
			...
			if (window.location.port === '8082') {
				...
				var image = message.substring(index, message.length);
				console.log('Image Link ::::::::::::::' + image);
				image_link = 'http://' + window.location.hostname + ':'
						+ window.location.port + '/VCS/PhotoImage/MessageImage'
						+ image;
	...
	function clickRoom(val1, val2, val3) {
		showLoadingImg();
		isRoomChange = true;
		$("#msgContent").show();
		$("#message").text('');
	...
	function changeFinishFlag() {
		phoneNo = null;
		showLoadingImg();
		console.log("hello changeFinishFlag is started");
